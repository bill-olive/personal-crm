...
export class GmailConnector {
  ...
  async listEmails(query: string, maxResults = 20): Promise<GmailMessage[]> {
    try {
      ...
      throw new Error(`Gmail listEmails failed: ${message}`);
    }
  }
  ...
  async getEmail(messageId: string): Promise<GmailMessage | null> {
    try {
      ...
      throw new Error(`Gmail getEmail failed: ${message}`);
    }
  }
  ...
  async sendEmail(to: string, subject: string, body: string): Promise<string> {
    try {
      ...
      throw new Error(`Gmail sendEmail failed: ${message}`);
    }
  }
  ...
  async draftEmail(to: string, subject: string, body: string): Promise<string> {
    try {
      ...
      throw new Error(`Gmail draftEmail failed: ${message}`);
    }
  }

  /**
   * Get all threads for a contact email.
   */
  async getThreads(contactEmail: string, maxResults = 20): Promise<GmailThread[]> {
    try {
      ...
      for (const thread of threads) {
        if (thread.id) {
          const full = await this.getThread(thread.id);
          if (full) results.push(full);
        }
      }
      ...
      throw new Error(`Gmail getThreads failed: ${message}`);
    }
  }
  ...
  async getThread(threadId: string): Promise<GmailThread | null> {
    try {
      ...
      throw new Error(`Gmail getThread failed: ${message}`);
    }
  }
